...
from datasets.slowfast_utils import pack_pathway_output
...
def params_count(model):
    ...
    Args:
        model (model_utils): model_utils to count the number of parameters.
    """
...
def log_model_info(model, cfg, is_train=True):
    ...
    Args:
        model (model_utils): model_utils to log the info.
        cfg (CfgNode): configs. Details can be found in
            slowfast/config/defaults.py
...
def is_eval_epoch(cfg, cur_epoch):
    """
    Determine if the model_utils should be evaluated at the current epoch.
    Args:
        cfg (CfgNode): configs. Details can be found in
            slowfast/config/defaults.py
...
def frozen_bn_stats(model):
    ...
    Args:
        model (model_utils): model_utils to set bn layers to eval mode.
    """
